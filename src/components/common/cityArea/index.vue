<template>
  <el-cascader filterable :class="{'cascader-input' : initValueFlag}" :placeholder="initValue" v-model="value" :options="cascader_options" :props="cascader_props" @change="changeValue">
  </el-cascader>
</template>
    ...
    return {
      ...
      cascader_props: {
        ...
        lazyLoad(node, resolve) {
          ...
          //如果不是第一层，那么将node的名称赋值给requestData，让requestData去请求数据
          ...
          GetCity(requestData).then((resonse) => {
            //返回的数据 CITY_CODE: "110100" CITY_ID: "1" CITY_NAME: "北京市"
